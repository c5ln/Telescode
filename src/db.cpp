...
static const char* kInitSQL =
    ...
    "    start_line    INTEGER NOT NULL,"
    "    end_line      INTEGER NOT NULL,"
    "    parent_type   TEXT NOT NULL CHECK(parent_type IN ('file', 'class'))"
    ");"
    "CREATE INDEX IF NOT EXISTS idx_function_parent ON function(parent_id);"
    "CREATE INDEX IF NOT EXISTS idx_function_file   ON function(parent_id) WHERE parent_type = 'file';"
