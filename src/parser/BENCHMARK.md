...
`parseDirectory`를 워커 풀(스레드 수 = `hardware_concurrency`, 12) + 아토믹 인덱스로
병렬화한 후, baseline과 동일한 방법으로 측정:

| 단계      | baseline (순차) | 병렬화 후          | 개선        | 
|-----------|-----------------|--------------------|-------------|
| 파싱      | ~2,220 ms       | ~377 ms (375–381) | **5.9배**   |
| DB insert | ~900 ms         | ~887 ms            | 변화 없음   |
| 전체      | ~3,120 ms       | ~1,264 ms          | **2.5배**   |

- 정확성: 병렬화 전/후 stdlib 스캔 DB 전체 테이블 정렬 덤프 109,034줄 **완전 일치**
- 12코어 대비 5.9배(이론치의 절반)인 이유: 파일 크기 편차로 인한 꼬리 지연,
  메모리 할당 경합 등. 현재로 충분하여 추가 튜닝은 보류.
- 이후 병목은 DB insert(전체의 70%)로 이동.
